const ManiaEngine = {
	...
	drawStage: function (startX, lanesNum) {
		...
		if (this.noteType === 'bar') {
			...
			for (let i = 0; i < 4; i++) {
				this.drawCircle(startX + (i * this.laneWidth) + this.laneWidth / 2, this.hitPosition + 10, (this.laneWidth * 0.80) / 2, "rgba(255, 255, 255, 0.5)");
			}
		}
	},
	...
	drawCircleNote: function (x, lane, y, color) {
		...
		this.drawCircle(actualX, y, (this.laneWidth * 0.80) / 2, color);
	},
	drawCircleLongNote: function (x, lane, startY, endY, color) {
		let actualX = (x + lane * this.laneWidth) + this.laneWidth / 2;
		let noteBodyX = (x + lane * this.laneWidth) + this.laneWidth * 0.1;
		let y = Math.min(startY, endY);
		let height = Math.abs(endY - startY);
		this.drawCircle(actualX, startY, (this.laneWidth * 0.80) / 2, color);
		this.drawCircle(actualX, endY, (this.laneWidth * 0.80) / 2, color);
		this.ctx.fillStyle = color;
		this.ctx.fillRect(noteBodyX, endY, this.laneWidth * 0.80, height);
	},
