const ManiaEngine = {
			...
			if (isHitObjectsSection) {
				...
				if (parts.length >= 3) {
					...
					if ((type & 128) === 128) {
						...
						this.activeNotes.push({ lane: lane, time: time, y: -100, hit: false });
						this.activeNotes.push({ lane: lane, time: endTime, y: -100, hit: false });
					} else {
						this.activeNotes.push({ lane: lane, time: time, y: -100, hit: false });
					}
				}
	...
	drawNote: function (startX, lane, y, height, color) {
		let x = startX + lane * this.laneWidth + 2;
		let w = this.laneWidth - 4;
