const ManiaEngine = {
	...
	drawStage: function (startX, lanesNum) {
		...
		if (this.noteType === 'bar') {
			this.ctx.fillStyle = "rgba(255, 255, 255, 0.5)";
			this.ctx.fillRect(startX, this.hitPosition, lanesNum * this.laneWidth, 20);
		} else if (this.noteType === 'circle') {
			for (let i = 0; i < 4; i++) {
				this.drawCircle(startX + (i * this.laneWidth) + this.laneWidth / 2, this.hitPosition + 10, (this.laneWidth * 0.9) / 2, "rgba(255, 255, 255, 0.5)");
			}
		}
	},
	...
	drawCircleNote: function (x, lane, y, color) {
		let actualX = (x + lane * this.laneWidth) + this.laneWidth / 2;
		this.drawCircle(actualX, y, (this.laneWidth * 0.9) / 2, color);
	},
	drawCircleLongNote: function (x, lane, startY, endY, color) {
		let actualX = (x + lane * this.laneWidth) + this.laneWidth / 2;
		let noteBodyX = (x + lane * this.laneWidth) + this.laneWidth * 0.05;
		let y = Math.min(startY, endY);
		let height = Math.abs(endY - startY);
		this.drawCircle(actualX, startY, (this.laneWidth * 0.9) / 2, color);
		this.drawCircle(actualX, endY, (this.laneWidth * 0.9) / 2, color);
		this.ctx.fillStyle = color;
		this.ctx.fillRect(noteBodyX, endY, this.laneWidth * 0.9, height);
	},
		...
		try {
			...
			for (let filename of Object.keys(zip.files)) {
				...
				if (cleanName.endsWith(".osu")) {
					...
					if (fileSize > largestImageSize) {
						largestImageSize = fileSize;
						songData.imageBlob = new Blob([uint8Array], { type: "image/jpeg" });
						console.log("extracted cover art: " + filename + " (" + fileSize + " bytes)");
					} else {
						console.log("ignored smaller image asset: " + filename + " (" + fileSize + " bytes)");
					}
